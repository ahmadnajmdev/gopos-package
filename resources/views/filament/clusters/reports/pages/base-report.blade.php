<x-filament-panels::page>
    <div class="space-y-6">
        ...
        @if($this->data && isset($this->data['startDate']) && isset($this->data['endDate']))
            @php
                        ...
                        <div class="px-5 py-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                            ...
                            <div class="text-lg font-bold text-gray-900 dark:text-white">
                                @if(is_array($data) && isset($data['sections']))
                                    {{ collect($data['sections'])->sum(fn($s) => count($s['accounts'] ?? [])) }}
                ...
                <div class="overflow-x-auto" dir="{{ $direction }}">
                    <table class="w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @if(is_array($data) && isset($data['sections']))
                                {{-- Balance Sheet / Sectioned Report Style --}}
                                @foreach($data['sections'] as $section)
                                    {{-- Section Header --}}
                                    <tr class="bg-gray-100 dark:bg-gray-700">
                                        <td colspan="{{ count($report->getColumns()) }}" class="px-6 py-3 text-sm font-bold text-gray-900 dark:text-white uppercase">
                                            {{ $section['title'] }}
                                        </td>
                                    </tr>
                                    {{-- Section Accounts --}}
                                    @foreach($section['accounts'] ?? [] as $account)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                                            <td class="px-6 py-3 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100 {{ $isRtl ? 'pr-10' : 'pl-10' }}">
                                                {{ $account['code'] ?? '' }} - {{ $account['name'] ?? '' }}
                                            </td>
                                            <td class="px-6 py-3 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">
                                                <span class="tabular-nums">{{ number_format($account['balance'] ?? 0, 2) }}</span>
                                                <span class="text-gray-500 dark:text-gray-400 {{ $isRtl ? 'mr-1' : 'ml-1' }}">{{ $section['currency'] ?? '' }}</span>
                                            </td>
                                        </tr>
                                    @endforeach
                                    {{-- Extra rows (like Net Income) --}}
                                    @foreach($section['extra_rows'] ?? [] as $extraRow)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150 bg-gray-50/50 dark:bg-gray-800/50">
                                            <td class="px-6 py-3 whitespace-nowrap text-sm font-medium text-gray-700 dark:text-gray-300 {{ $isRtl ? 'pr-10' : 'pl-10' }} italic">
                                                {{ $extraRow['name'] ?? '' }}
                                            </td>
                                            <td class="px-6 py-3 whitespace-nowrap text-sm font-medium text-gray-700 dark:text-gray-300 italic">
                                                <span class="tabular-nums">{{ number_format($extraRow['balance'] ?? 0, 2) }}</span>
                                                <span class="text-gray-500 dark:text-gray-400 {{ $isRtl ? 'mr-1' : 'ml-1' }}">{{ $section['currency'] ?? '' }}</span>
                                            </td>
                                        </tr>
                                    @endforeach
                                    ...
                                    <tr class="bg-gray-200 dark:bg-gray-600">
                                        <td class="px-6 py-3 whitespace-nowrap text-sm font-bold text-gray-900 dark:text-white">
                                            {{ __('Total') }} {{ $section['title'] }}
                                        </td>
                                        <td class="px-6 py-3 whitespace-nowrap text-sm font-bold text-gray-900 dark:text-white">
                                            <span class="tabular-nums">{{ number_format($section['total'] ?? 0, 2) }}</span>
                                            <span class="text-gray-500 dark:text-gray-400 {{ $isRtl ? 'mr-1' : 'ml-1' }}">{{ $section['currency'] ?? '' }}</span>
                                        </td>
                                    </tr>
                                @endforeach
                                ...
                                @if(isset($data['summary']))
                                    <tr class="bg-primary-100 dark:bg-primary-900/30 border-t-2 border-primary-300 dark:border-primary-700">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-primary-900 dark:text-primary-100">
                                            {{ __('Total Assets') }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-primary-900 dark:text-primary-100">
                                            <span class="tabular-nums">{{ number_format($data['summary']['total_assets'] ?? 0, 2) }}</span>
                                            <span class="text-primary-600 dark:text-primary-400 {{ $isRtl ? 'mr-1' : 'ml-1' }}">{{ $data['summary']['currency'] ?? '' }}</span>
                                        </td>
                                    </tr>
                                    <tr class="bg-primary-100 dark:bg-primary-900/30">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-primary-900 dark:text-primary-100">
                                            {{ __('Total Liabilities + Equity') }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-primary-900 dark:text-primary-100">
                                            <span class="tabular-nums">{{ number_format($data['summary']['liabilities_plus_equity'] ?? 0, 2) }}</span>
                                            <span class="text-primary-600 dark:text-primary-400 {{ $isRtl ? 'mr-1' : 'ml-1' }}">{{ $data['summary']['currency'] ?? '' }}</span>
                                        </td>
                                    </tr>
                                    <tr class="{{ ($data['summary']['is_balanced'] ?? false) ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30' }}">
                                        <td colspan="2" class="px-6 py-4 text-center text-sm font-bold {{ ($data['summary']['is_balanced'] ?? false) ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300' }}">
                                            @if($data['summary']['is_balanced'] ?? false)
                                                ✓ {{ __('Balance Sheet is balanced') }}
                                            @else
                                                ✗ {{ __('Balance Sheet is NOT balanced') }}
                                            @endif
                                        </td>
                                    </tr>
                                @endif
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm {{ $row['is_total'] ?? false ? 'font-bold' : ($row['is_subtotal'] ?? false ? 'font-semibold' : 'font-medium') }} {{ $key === 'amount' && isset($row['amount']) && $row['amount'] < 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-gray-100' }} {{ $key === 'amount' && isset($row['amount']) && $row['amount'] >= 0 && ($row['is_total'] ?? false) ? 'text-green-600 dark:text-green-400' : '' }}">
                                                @if($column['type'] === 'currency')
                                                    <span class="inline-flex items-center">
                                                        ...
                                                        @endif
                                                        <span class="tabular-nums">{{ number_format($row[$key] ?? 0, 2) }}</span>
                            ...
                            @else
                                ...
                                @foreach($data as $index => $row)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150 {{ $loop->index % 2 === 0 ? '' : 'bg-gray-50/50 dark:bg-gray-800/50' }}">
                                        @foreach($report->getColumns() as $key => $column)
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">
                                                ...
                                                @elseif($column['type'] === 'number')
                                                    <span class="tabular-nums">{{ number_format($row[$key], 2) }}</span>
                                                    @if(isset($row[$key . '_suffix']) || isset($column['suffix']))
                                                        <span class="text-gray-500 dark:text-gray-400 {{ $isRtl ? 'mr-1' : 'ml-1' }}">{{ $row[$key . '_suffix'] ?? $column['suffix'] }}</span>
                                                    @endif
                                ...
                                @if($report->shouldShowTotals() && !empty($totals))
                                    <tr class="bg-primary-50 dark:bg-primary-900/20 border-t-2 border-primary-200 dark:border-primary-700">
                                        @php
                                            ...
                                            $currency = $firstRow['currency'] ?? \Gopos\Models\Currency::getBaseCurrency()?->symbol;
                                        @endphp
                                        @foreach($report->getColumns() as $key => $column)
                                            @if($firstColumn)
                                            ...
                                            @elseif(in_array($key, $report->getTotalColumns()))
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 dark:text-white">
                                                    <span class="inline-flex items-center">
                                                        ...
                                                        @elseif(isset($column['suffix']))
                                                            <span class="text-gray-500 dark:text-gray-400 {{ $isRtl ? 'mr-1' : 'ml-1' }}">{{ $column['suffix'] }}</span>
                                                        @endif
                                                    </span>
                                                </td>
                                            @endif
                ...
                @php
                    ...
                    if (is_array($data)) {
                        if (isset($data['sections'])) {
                            $isEmpty = collect($data['sections'])->every(fn($s) => empty($s['accounts'] ?? []));
                        } elseif (isset($data['rows'])) {
                            $isEmpty = empty($data['rows']);
                        } else {
                            $isEmpty = empty($data);
                        }
                    } else {
            ...
            <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
                <div class="flex items-center justify-between text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <div class="flex items-center gap-4">
                        @if(!is_array($data) || (!isset($data['rows']) && !isset($data['sections'])))
                            <span class="inline-flex items-center">
                                <svg class="w-4 h-4 {{ $isRtl ? 'ml-1.5' : 'mr-1.5' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                </svg>
                                {{ is_array($data) ? count($data) : $data->count() }} {{ __('records') }}
                            </span>
                        @endif
                    </div>
                </div>
        ...
        @else
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-12">
                <div class="text-center">
                    ...
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">{{ __('Choose a date range to generate your report.') }}</p>
                </div>
            </div>
        @endif
    </div>
