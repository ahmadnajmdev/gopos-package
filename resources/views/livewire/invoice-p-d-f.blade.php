@php
    ...
    $textAlignStart = $dir === 'rtl' ? 'right' : 'left';
    $textAlignEnd = $dir === 'rtl' ? 'left' : 'right';
    $paddingStart = $dir === 'rtl' ? 'pr' : 'pl';
    $paddingEnd = $dir === 'rtl' ? 'pl' : 'pr';
    // Currency symbol (fallback to code, then base currency)
    $baseCurrency = \Gopos\Models\Currency::getBaseCurrency();
    $currencySymbol = $purchase->currency?->symbol ?? $purchase->currency?->code ?? $baseCurrency?->symbol;
    // Currency decimal places (fallback to 0)
    $currencyDecimals = $purchase->currency?->decimal_places ?? 0;
@endphp
...
<div dir="{{ $dir }}" class="font-custom max-w-4xl p-8 mx-auto my-8 bg-white rounded-lg "
    ...
    <div class="flex items-center justify-between pb-6 mb-6 border-b border-gray-200">
        <div class="text-{{ $textAlignStart }}">
            ...
            <p class="text-sm text-gray-500">{{ __('Invoice') }} #{{ $purchase->purchase_number }}</p>
            <p class="text-sm text-gray-500">{{ __('Date') }}: {{ $purchase->purchase_date }}</p>
        </div>
    ...
    <div class="flex justify-between mb-8">
        <div class="w-1/2 {{ $paddingEnd }}-4 text-{{ $textAlignStart }}">
            ...
            <p class="text-sm text-gray-600">{{ $purchase->supplier?->address ?? '' }}</p>
            @if ($purchase->supplier?->phone)
                <p class="text-sm text-gray-600">{{ __('Phone') }}: {{ $purchase->supplier->phone }}</p>
            @endif
            @if ($purchase->supplier?->email)
                <p class="text-sm text-gray-600">{{ __('Email') }}: {{ $purchase->supplier?->email ?? '' }}</p>
            @endif
        </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($purchase->items as $item)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-gray-700 whitespace-nowrap text-{{ $textAlignStart }}">
                            {{ Number::format($item->cost, $currencyDecimals, locale: 'en') }} {{ $currencySymbol }}</td>
                        <td class="px-4 py-3 text-gray-700 whitespace-nowrap text-{{ $textAlignStart }}">
                            {{ Number::format($item->stock * $item->cost, $currencyDecimals, locale: 'en') }} {{ $currencySymbol }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="4" class="px-4 py-10 text-center text-gray-500">No items in this purchase.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="flex justify-end mb-8 text-sm text-gray-700">
        <div class="w-full max-w-xs text-{{ $textAlignEnd }}">
            <div class="flex justify-between py-1">
                ...
                <span>{{ Number::format($purchase->sub_total, $currencyDecimals, locale: 'en') }} {{ $currencySymbol }}</span>
            </div>
            <div class="flex justify-between py-1">
                <span>{{ __('Discount amount') }}</span>
                <span>-{{ Number::format($purchase->discount_amount, $currencyDecimals, locale: 'en') }} {{ $currencySymbol }}</span>
            </div>
            {{-- Optional Tax --}}
            {{-- <div class="flex justify-between py-1">
                <span>Tax (10%)</span>
                <span>{{ Number::format(($purchase->sub_total - $purchase->discount_amount) * 0.10, $currencyDecimals, locale: 'en') }} {{ $currencySymbol }}</span>
            </div> --}}
            <div class="flex justify-between py-2 mt-2 font-semibold border-t-2 border-gray-200">
                <span>{{ __('Total amount') }}</span>
                <span>{{ Number::format($purchase->sub_total - $purchase->discount_amount, $currencyDecimals, locale: 'en') }}
                    {{ $currencySymbol }}</span>
            </div>
            {{-- Optional Paid/Due --}}
            @if (isset($purchase->paid_amount))
                <div class="flex justify-between py-1 mt-2 text-green-600">
                    ...
                    <span>{{ Number::format($purchase->paid_amount, $currencyDecimals, locale: 'en') }} {{ $currencySymbol }}</span>
                </div>
            @endif
            @if (isset($purchase->amount_due))
                <div
                    class="flex justify-between py-1 font-medium {{ $purchase->amount_due > 0 ? 'text-red-600' : 'text-gray-700' }}">
                    <span>{{ __('Due Amount') }}</span>
                    <span>{{ Number::format($purchase->amount_due, $currencyDecimals, locale: 'en') }} {{ $currencySymbol }}</span>
                </div>
            @endif
        </div>
    </div>
    ...
    @if (request()->routeIs('print-invoice'))
    <script>
